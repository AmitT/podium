...
class Settings extends PodiumCoreProperties
{
    public function displaySidebar()
    {
        ...
        if (in_array($this->templateUrl, $rules['excludeByFileName'], true)) {
            ...
            isset($postType) &&
            in_array($postType, $rules['excludeByPostByType'], true)
        ) {
            return false;
        } elseif (
            isset($taxonomyID) &&
            in_array($taxonomyID, $rules['excludeByTaxonomyID'], true)
        ) {
            return false;
        } elseif (
            isset($postID) &&
            in_array($postID, $rules['excludeByPostID'], true)
        ) {
            return false;
        } elseif (
            isset($categoryID) &&
            in_array($categoryID, $rules['excludeByCategoryID'], true)
        ) {
            return false;
        } else {
            return true;
    ...
    public function getContentClass(
        $contentHasSidebarClass = 'medium-8',
        $contentNoSidebarClass = 'medium-12'
    ) {
        // return bool
        ...
        if (has_nav_menu('main-nav')) {
            ...
            if ('onCanvass' == $canvas) {
                ...
                $onCanvas = [
                    ...
                    'container' => false,
                    'items_wrap' =>
                        '<ul id="%1$s" class="%2$s show-for-medium" data-dropdown-menu>%3$s</ul>',
                    'container_class' => '',
                    'container_id' => '',
                ...
                $offCanvas = [
                    ...
                    'link_after' => '',
                    'items_wrap' =>
                        '<ul class="off-canvas-list %2$s" role="navigation">%3$s</ul>',
                    'depth' => 0,
                    'walker' => $walker_object
                ];
