...
if ( version_compare( $GLOBALS['wp_version'], '4.1', '<' ) ) :
	 ...
	 */
	function podium_wp_title( $title, $sep ) {
		if ( is_feed() ) {
			return $title;
		}
		...
		if ( ( $paged >= 2 || $page >= 2 ) && ! is_404() ) {
			$title .= " $sep " . sprintf( esc_html__( 'Page %s', 'podium' ), max( $paged, $page ) );
		}

		return $title;
	}
	add_filter( 'wp_title', 'podium_wp_title', 10, 2 );
